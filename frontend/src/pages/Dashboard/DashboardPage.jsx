...
import { Play, TrendingUp, Activity, Zap, Info, Search, Globe, Coins, Shield, MousePointerClick, Crosshair, AlertTriangle } from 'lucide-react';
...
const DashboardPage = () => {
    ...
    const [eliteSignals, setEliteSignals] = useState([]); // NEW: Store Top 5
    const [isScanning, setIsScanning] = useState(false);
    const [comparisonData, setComparisonData] = useState(null);

    // --- SETUP CHART (Sama seperti sebelumnya) ---
    useEffect(() => {
        if (!priceChartContainer.current) return;
    ...
    const handleScanMarket = async () => {
        setIsScanning(true);
        setScanResults({});
        setEliteSignals([]); // Reset Elite Signals
        
        let allElites = [];

        for (const sector of SECTORS_LIST) {
            try {
                // Gunakan default period 1y untuk scan dasar
                ...
                if (res.ok) {
                    ...
                    // Kumpulkan Elite Signals dari setiap sektor
                    if(data.elite_signals) {
                        allElites = [...allElites, ...data.elite_signals];
                    }
                }
            } catch (err) { console.error(`Gagal scan ${sector.name}`, err); }
        }
        
        // Final Sort & Slice Top 5 Global
        allElites.sort((a,b) => b.win_rate - a.win_rate || b.trades - a.trades);
        setEliteSignals(allElites.slice(0, 5));
        ...
        <div className="min-h-screen bg-black text-cyan-50 font-sans p-4 md:p-8 selection:bg-cyan-500 selection:text-black pb-20">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-center mb-8 border-b border-cyan-900/50 pb-6 gap-6 relative">
                ...
                <div>
                    ...
                    <p className="text-[10px] md:text-xs text-gray-400 font-mono tracking-[0.2em] mt-1 uppercase opacity-80">BY <span className="text-cyan-300">RAFLYANDI ALVIANSYAH</span> & <span className="text-purple-300">BRIAN EVAN KRISTANTO</span></p>
                </div>
                <div className="flex flex-col gap-2 items-center">
                    <div className="flex flex-wrap gap-3 bg-gray-900/40 backdrop-blur-md p-4 rounded-xl border border-cyan-500/30 shadow-[0_0_20px_rgba(8,145,178,0.2)] justify-center items-end">
                        ...
                        <button onClick={handleRunBacktest} disabled={loading} className="mt-auto h-[42px] bg-gradient-to-r from-cyan-600 to-blue-700 hover:from-cyan-500 hover:to-blue-600 text-white px-6 rounded-lg font-bold flex items-center gap-2 transition-all shadow-[0_0_15px_rgba(8,145,178,0.5)] disabled:opacity-50">
                    ...
                    <div className="bg-gray-900/30 border border-gray-800 rounded-2xl p-6">
                        ...
                        <div className="space-y-3 font-mono text-sm">
                            ...
                            <div className="flex justify-between items-center text-sm border-b border-gray-800 pb-2 hover:bg-white/5 px-2 rounded transition">
                                <span className="text-gray-500 text-xs uppercase flex items-center">Sharpe Ratio <InfoTooltip text="Reward per unit of risk. Higher (>1) is better. Measured against StdDev."/></span>
                                <span className="font-bold text-yellow-400">{metrics?.sharpe_ratio || 0}</span>
                            </div>

                            <div className="flex justify-between items-center text-sm border-b border-gray-800 pb-2 hover:bg-white/5 px-2 rounded transition">
                                <span className="text-gray-500 text-xs uppercase flex items-center">Calmar Ratio <InfoTooltip text="Annual Return / Max Drawdown. Higher is better recovery."/></span>
            ...
            {/* --- GLOBAL SCANNER & ELITE SIGNALS --- */}
            <div className="bg-gray-900/20 border border-cyan-900/30 rounded-3xl p-8 relative overflow-hidden">
                ...
                {/* --- TOP 5 ELITE SIGNALS (NEW) --- */}
                {eliteSignals.length > 0 && (
                    <div className="mb-8 animate-in slide-in-from-top-4 fade-in duration-500">
                        <h3 className="text-lg font-bold text-yellow-400 mb-4 flex items-center gap-2"><AlertTriangle size={18} /> ELITE SIGNALS (WR {'>'} 70%)</h3>
                        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                            {eliteSignals.map((sig, idx) => (
                                <div key={idx} className="bg-yellow-900/10 border border-yellow-500/50 p-4 rounded-xl relative overflow-hidden hover:bg-yellow-900/20 transition cursor-pointer" onClick={() => {
                                    ...
                                    <div className="text-xs text-gray-500 mb-3 italic">{sig.strategy}</div>
                                    
                                    {/* SIGNAL RECOMMENDATION */}
                                    <div className="bg-black/40 p-2 rounded border border-gray-800 text-[10px] space-y-1 font-mono">
                                        <div className="text-gray-400 text-center border-b border-gray-700 pb-1 mb-1">Signal Setup Suggestion</div>
                                        <div className="flex justify-between"><span className="text-blue-400">ENTRY</span> <span>${sig.signal_data.price.toFixed(2)}</span></div>
                                        <div className="flex justify-between"><span className="text-green-400">TP (Long)</span> <span>${sig.signal_data.setup_long.tp.toFixed(2)}</span></div>
                                        <div className="flex justify-between"><span className="text-red-400">SL (Long)</span> <span>${sig.signal_data.setup_long.sl.toFixed(2)}</span></div>
                                    </div>
                                    <div className="mt-2 text-center text-cyan-500 text-[10px] font-bold tracking-widest hover:underline">CLICK TO LOAD</div>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {/* HASIL SCAN PER SEKTOR (Updated Display) */}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                {scanResults[sector.name] ? scanResults[sector.name].map((coin, idx) => (
                                    <div key={idx} className="bg-black/40 border border-gray-800 p-4 rounded-xl hover:border-cyan-500/50 transition group cursor-pointer" onClick={() => {
                                        ...
                                        <div className="mt-2 pt-2 border-t border-gray-800 text-[10px] text-cyan-300 italic flex items-center gap-1">
                                            <MousePointerClick size={10}/> {coin.reason || "Analyzing..."}
                                        </div>
                                    </div>
                                )) : (isScanning ? <div className="col-span-4 text-xs text-gray-600 animate-pulse">Scanning sector best combinations...</div> : <div className="col-span-4 text-xs text-gray-700">Waiting to scan...</div>)}
                            </div>
                        </div>
                    ))}
                </div>
